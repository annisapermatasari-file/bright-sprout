...
/**
 * COUNT_CIRCLE: same data shape as OptionChoiceActivity again, presented as
 * round badges the child "circles" (a thick ring appears) instead of a
 * rectangular button lighting up — matching worksheets like "Circle the
 * Correct Number".
 */
export function CircleSelectActivity({ question, answerState, onSelect, disabled }: ActivityComponentProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-8">
      ...
      <div className="circle-options" role="group" aria-label="Lingkari jawaban yang benar">
        {options.map((option) => {
          ...
          const isPicked = answerState.status === "answered" && answerState.selectedOptionId === option.id;
          return (
            <button
              key={option.id}
              type="button"
              ...
              className={cn(
                "circle-option reveal answer-option-transition",
                "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
                !locked && "hover-scale",
                answered && !isPicked && !isRevealedCorrect && "opacity-40",
                isRevealedCorrect && "celebrate circle-option-correct",
                isWrongSelection && "shake circle-option-wrong",
              )}
            >
              {option.label}
            </button>
